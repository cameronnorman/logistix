...
class BookingInstance
  ...
  def save
    day_bookings = Booking.find_by_date(start_date_time.to_date)
    existing_booking = day_bookings.select do |booking|
      booking.start_date_time == start_date_time && booking.end_date_time == end_date_time
    end

    return false if existing_booking.any?

    BOOKINGS << as_json.symbolize_keys
    true
  end

  def available?
    # Check if any of the current bookings on the specified date overlap with this booking
    possible_booking_range = start_date_time..end_date_time
    current_booking_as_ranges.select do |current_booking_range|
      ((current_booking_range.first - possible_booking_range.end) * (possible_booking_range.first - current_booking_range.end)).positive?
    end.none?
  end
  ...
  def current_booking_as_ranges
    current_bookings = Booking.find_by_date(start_date_time.to_date)
