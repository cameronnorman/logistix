class Booking
	class << self
		...
		def find_avaliabilities(date, delivery_interval)
			...
			available_slots = booking_slots.select{|booking| booking.available?}
		...
		def generate_booking_slots(date, delivery_interval)
			...
			interval_slots_as_times.map{|start_date_time, end_date_time| BookingInstance.new(nil, start_date_time, end_date_time)}
		end
	end
end
